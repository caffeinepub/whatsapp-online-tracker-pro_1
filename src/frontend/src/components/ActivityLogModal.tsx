...
import { Activity, Clock, Download, Phone } from "lucide-react";
...
export function ActivityLogModal({
  ...
  const { getCallHistory } = useCallHistory();
              ...
              {entries.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full py-12 text-center px-6">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Status changes will appear here automatically
                  </p>
                </div>
              ) : (
                  ...
                  {entries.map((entry, idx) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {/* sessionDurationMs from manual toggle (when duration not set) */}
                        {entry.status === "offline" &&
                          entry.duration == null &&
          ...
          <TabsContent value="calls" className="mt-0">
            <div className="px-5 py-2.5 border-b border-border flex items-center justify-between">
              ...
              <div className="flex gap-1.5">
                <Badge variant="secondary" className="text-xs">
                  {callRecords.filter((r) => r.type === "audio").length} audio
                </Badge>
                <Badge variant="outline" className="text-xs">
                  {callRecords.filter((r) => r.type === "video").length} video
                </Badge>
              </div>
            </div>
            <ScrollArea className="h-72 scrollbar-thin">
              <CallHistorySection records={callRecords} />
            </ScrollArea>
          </TabsContent>
        </Tabs>
      </DialogContent>
